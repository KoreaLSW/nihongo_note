...
import { deleteFromNote, setMemorized, setReviewed } from '@/app/actions/note';
...
export function VocabularyCard({
    ...
    const levelStyle = levelClass[row.level?.toUpperCase() ?? ''] ?? 'inline-block w-fit rounded-md bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-700 dark:text-zinc-300';

    return (
        <Link
            href={`/vocabulary/${row.no}`}
            className='relative flex min-h-[160px] flex-col justify-between rounded-xl border border-zinc-200 bg-white p-4 shadow-sm transition hover:shadow dark:border-zinc-700 dark:bg-zinc-800/50 dark:hover:bg-zinc-800'
        >
            <form
