...
import { JlptWordbooksReorderList } from "./JlptWordbooksReorderList";

type Props = {
  level: JlptLevel;
  wordbooks: JlptWordbookMeta[];
};

export function JlptWordbooksReorderPanel({ level, wordbooks }: Props) {
  ...
  return (
    <div className="mb-8 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-800/50">
      ...
      <div id={panelId} className={open ? "mt-4" : "mt-4 hidden"}>
        <JlptWordbooksReorderList level={level} initialWordbooks={wordbooks} />
      </div>
    </div>
  );
}
