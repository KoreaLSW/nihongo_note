...
import { renameJlptWordbookAction } from "@/app/actions/jlptWordbook";
...
export function RenameJlptWordbookForm({ wordbookId, initialName }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    setPending(true);
    const res = await renameJlptWordbookAction(new FormData(e.currentTarget));
    setPending(false);
    if (res?.ok) {
      router.refresh();
    } else {
