...
import { getJlptMemorizedForQuizView, type JlptWordbookRow } from "@/lib/jlptWordbookShared";
...
export function JlptQuizCard({ row, wordbookId, quizView, variant, kanjiLines = [] }: Props) {
  ...
  const setMemorized = async (value: "yes" | "no") => {
    ...
    await setJlptWordMemorizedAction(fd);
    router.refresh();
  };

  const modeMemorized = getJlptMemorizedForQuizView(row, quizView);
