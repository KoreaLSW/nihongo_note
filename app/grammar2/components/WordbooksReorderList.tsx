...
import { reorderGrammarWordbooksAction } from "@/app/actions/grammarWordbook";

type Props = {
  initialWordbooks: GrammarWordbookMeta[];
};

function SortableWordbookItem({ wb }: { wb: GrammarWordbookMeta }) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export function WordbooksReorderList({ initialWordbooks }: Props) {
  const router = useRouter();
  const [items, setItems] = useState<GrammarWordbookMeta[]>(initialWordbooks);
  ...
  const handleSave = async () => {
    setError(null);
    setSaving(true);
    const res = await reorderGrammarWordbooksAction(items.map((w) => w.id));
    setSaving(false);
    if (res?.ok) {
      router.refresh();
    } else {
