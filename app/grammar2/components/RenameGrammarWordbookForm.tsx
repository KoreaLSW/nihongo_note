...
import { renameGrammarWordbookAction } from "@/app/actions/grammarWordbook";
...
export function RenameGrammarWordbookForm({
  wordbookId,
  initialName,
}: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    setPending(true);
    const res = await renameGrammarWordbookAction(new FormData(e.currentTarget));
    setPending(false);
    if (res?.ok) {
      router.refresh();
    } else {
