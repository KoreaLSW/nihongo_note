...
import type { GrammarWordbookRow } from "@/lib/grammarWordbook";
import { reorderGrammarWordbookWordsAction } from "@/app/actions/grammarWordbook";

type Props = {
  wordbookId: string;
  initialWords: GrammarWordbookRow[];
};

function SortableGrammarItem({ row }: { row: GrammarWordbookRow }) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export function GrammarReorderList({ wordbookId, initialWords }: Props) {
  const router = useRouter();
  const [items, setItems] = useState<GrammarWordbookRow[]>(initialWords);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    setItems((prev) => {
      const oldIndex = prev.findIndex((r) => r.grammar === active.id);
      const newIndex = prev.findIndex((r) => r.grammar === over.id);
  ...
  const handleSave = async () => {
    setError(null);
    setSaving(true);
    const res = await reorderGrammarWordbookWordsAction(
      wordbookId,
      items.map((r) => r.grammar)
    );
    setSaving(false);
    if (res?.ok) {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <SortableContext
          items={items.map((r) => r.grammar)}
          strategy={verticalListSortingStrategy}
        >
          <div className="flex flex-col gap-2">
            {items.map((row) => (
              <SortableGrammarItem key={row.grammar} row={row} />
            ))}
          </div>
        </SortableContext>
      </DndContext>
