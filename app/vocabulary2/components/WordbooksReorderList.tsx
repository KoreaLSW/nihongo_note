...
import { reorderWordbooksAction } from "@/app/actions/wordbook";

type Props = {
  initialWordbooks: WordbookMeta[];
};

function SortableWordbookItem({ wb }: { wb: WordbookMeta }) {
...
export function WordbooksReorderList({ initialWordbooks }: Props) {
  ...
  const [items, setItems] = useState<WordbookMeta[]>(initialWordbooks);
  ...
  const handleSave = async () => {
    setError(null);
    setSaving(true);
    const res = await reorderWordbooksAction(items.map((w) => w.id));
    setSaving(false);
    if (res?.ok) {
      router.refresh();
    } else {
